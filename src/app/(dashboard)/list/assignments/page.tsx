...
const AssignementsList = () => {

    const renderRow = (item:Assignements)=> (
        <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
            ...
            <td className="hidden md:table-cell px-2">{item.dueDate}</td>
            <td>
                <div className="flex items-center gap-2">
                <Link href={`/list/Assignementss/${item.id}`}>
                ...
                {role === "admin" && (
                    ...
                    // <FormModal table="Assignements" type="delete" id={item.id}/>
                )}
                </div>
            </td>
        </tr>
    ...
    return (
      <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
        ...
        <div className='flex items-center justify-between'>
            <h2 className="hidden md:block text-lg font-semibold">All Assignements</h2>
  ...
  export default AssignementsList